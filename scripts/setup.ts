...
export const normalizeQuranCompilation = (compilation: RawCompilation, collection: Collection): LoadedCompilation => {
    ...
    const normalizedExcerpts = compilation.excerpts.map((excerpt) => {
        ...
        if (!headingId) {
            throw new Error(`Qur'an excerpt is missing meta.headingId: ${excerpt.text.slice(0, 40)}`);
        }

        const matchedHeading = headingById.get(headingId);
        if (!matchedHeading) {
            throw new Error(`Qur'an excerpt references unknown headingId ${headingId}`);
        }

        const ayahNumber = (excerptCountByHeading.get(headingId) ?? 0) + 1;
        excerptCountByHeading.set(headingId, ayahNumber);

        const excerptId = excerpt.id ?? `${headingId}-${ayahNumber}`;
        ...
        return {
            ...
            nass: excerpt.nass,
            text: excerpt.text,
            from: typeof excerpt.from === 'number' ? excerpt.from : ayahNumber,
            translator: excerpt.translator ?? 0,
            lastUpdatedAt: excerpt.lastUpdatedAt ?? timestamp,
            meta: {
                surah: matchedHeading.surah,
                ayah: ayahNumber,
                surahName: matchedHeading.heading.text,
            },
        } satisfies Excerpt;
    });
